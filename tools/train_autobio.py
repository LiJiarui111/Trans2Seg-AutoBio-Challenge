...
from data.coco2017.utils.dataset import HOIDataset
from data.coco2017.utils.anno_read import anno_read
...
class Trainer(object):
    def __init__(self, args):
        ...
        #TODO change to AutoBio version
        # print(os.path.join(root_path, 'data\\coco2017\\utils\\anno_read.py'))
        ar = anno_read(annotation_file=os.path.join(root_path, 'data\\coco2017\\data\\anno\\Instance_segmentation.json'), mode='train')
        ...
        train_dataset = HOIDataset(anno_read=ar, insize=512, mode='train', task='instance_segmentation')
        val_dataset = HOIDataset(anno_read=ar, insize=512, mode='eval', task='instance_segmentation')
        test_dataset = HOIDataset(anno_read=ar, insize=512, mode='eval', task='instance_segmentation')
    ...
    def train(self):
        ...
        for (images, targets, _, aa) in self.train_loader:
            ...
            # print(images.shape)
            images = images.reshape(1, 3, 512, 512).float()
            ...
            # plt.imshow(targets.cpu().squeeze())
            # plt.show()
            # print((list(targets.cpu().squeeze().numpy())).any())
            # np.savetxt("ttt.txt", targets.cpu().squeeze()[2].numpy())
    ...
    def validation(self, epoch):
        ...
        model.eval()
        for i, (image, target, filename, _) in enumerate(self.val_loader):
            image = image.to(self.device)
            image = image.squeeze()
            # print(images.shape)
            image = image.reshape(1, 3, 512, 512).float()
    ...
    def test(self, vis=True):
        ...
        torch.cuda.empty_cache()
        model.eval()
        for i, (image, target, filename, _) in enumerate(self.test_loader):
            image = image.to(self.device)
            image = image.squeeze()
            # print(images.shape)
            image = image.reshape(1, 3, 512, 512).float()
            ...
            # plt.show()
            target_pred = target.cpu().squeeze().numpy()
            target_pred = get_color_pallete(target_pred, dataset="transparent11")
            target_save_path = os.path.join(cfg.TRAIN.MODEL_SAVE_DIR, 'gt')
            if not os.path.exists(target_save_path):
                os.makedirs(target_save_path)
            target_pred.save(os.path.join(target_save_path, str(i)+'.png'))
            print("[VIS TEST] Save GT Sample: {:d}".format(i+1))
            ...
            if vis:
                ...
                if save_gt:
                    ...
                    save_path = 'workdirs2/trans10kv2/gt_img'
                    ...
                    vis_gt = get_color_pallete(gt_mask, dataset="transparent11")
                    ...
                    # plt.imshow(vis_pred)
                    # plt.show()
                    vis_pred = get_color_pallete(vis_pred, dataset="transparent11")
